...
describe('remove ng-zorro font-face', () => {
  ...
  beforeEach(done => {
    new Promise((resolve, reject) => {
      find.file(/.js$/, path.join(__dirname, '../node_modules/ng-zorro-antd'), (files) => {
        const result = [];
        for (let f of files) {
          const content = fs.readFileSync(f);
          if (content.indexOf('@font-face') > -1 && content.indexOf('anticon') > -1) {
            result.push(f);
          }
        }
        resolve(result);
      ...
      .then((result) => {
        ...
        return Promise.all(toBeReplaceFile.map((s, index) => {
  ...
  afterEach(done => {
    Promise.all(toBeReplaceFile.map((s, index) => {
  ...
  it('should remove anticon font face', done => {
    ...
    replace
      .then(() => {
        return Promise.all(toBeReplaceFile.map((s) => {
          const content = fs.readFileSync(s);
